...
# ---------- SVG path data from the official AWS logo ----------

_TEXT_PATH = (
    "M86.4,66.4c0,3.7,0.4,6.7,1.1,8.9c0.8,2.2,1.8,4.6,3.2,7.2c0.5,0.8,0.7,1.6,0.7,2.3c0,1-0.6,2-1.9,3l-6.3,4.2"
...
_ARROW_CURVE_PATH = (
    "M273.5,143.7c-32.9,24.3-80.7,37.2-121.8,37.2c-57.6,0-109.5-21.3-148.7-56.7c-3.1-2.8-0.3-6.6,3.4-4.4"
    "c42.4,24.6,94.7,39.5,148.8,39.5c36.5,0,76.6-7.6,113.5-23.2C274.2,133.6,278.9,139.7,273.5,143.7z"
)

_ARROW_HEAD_PATH = (
    "M287.2,128.1c-4.2-5.4-27.8-2.6-38.5-1.3c-3.2,0.4-3.7-2.4-0.8-4.5c18.8-13.2,49.7-9.4,53.3-5"
    "c3.6,4.5-1,35.4-18.6,50.2c-2.7,2.3-5.3,1.1-4.1-1.9C282.5,155.7,291.4,133.4,287.2,128.1z"
)

# The arrow region spans roughly y=117–181.  Flip center ≈ 149.
_ARROW_FLIP_CENTER_Y = 149
_ARROW_FLIP_TRANSLATE = 2 * _ARROW_FLIP_CENTER_Y


def _build_svg(*, sad: bool, colour: bool, text_stroke_width: float = 0) -> str:
    text_fill = "#252F3E" if colour else "#000000"
    arrow_fill = "#FF9900" if colour else "#000000"

    text_stroke = ""
    if text_stroke_width > 0:
        text_stroke = (
            f' stroke="{text_fill}" stroke-width="{text_stroke_width}" stroke-linejoin="round"'
        )

    arrow_transform = ""
    if sad:
        arrow_transform = f' transform="translate(0,{_ARROW_FLIP_TRANSLATE}) scale(1,-1)"'

    return f"""\
<?xml version="1.0" encoding="utf-8"?>
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 304 182">
  <path fill="{text_fill}"{text_stroke} d="{_TEXT_PATH}"/>
  <g{arrow_transform}>
    <path fill="{arrow_fill}" fill-rule="evenodd" clip-rule="evenodd" d="{_ARROW_CURVE_PATH}"/>
    <path fill="{arrow_fill}" fill-rule="evenodd" clip-rule="evenodd" d="{_ARROW_HEAD_PATH}"/>
  </g>
</svg>"""


def _svg_to_pil(svg: str, width: int, height: int) -> PIL.Image.Image:
    """Render SVG to a Pillow RGBA image using macOS AppKit."""
    data = Foundation.NSData.dataWithBytes_length_(svg.encode(), len(svg.encode()))
    ns_image = AppKit.NSImage.alloc().initWithData_(data)
    ns_image.setSize_(Foundation.NSSize(width, height))

    bitmap_rep = AppKit.NSBitmapImageRep.alloc().initWithBitmapDataPlanes_pixelsWide_pixelsHigh_bitsPerSample_samplesPerPixel_hasAlpha_isPlanar_colorSpaceName_bytesPerRow_bitsPerPixel_(
        None,
        width,
        height,
        8,
    ...
    ctx = AppKit.NSGraphicsContext.graphicsContextWithBitmapImageRep_(bitmap_rep)
    AppKit.NSGraphicsContext.setCurrentContext_(ctx)
    ns_image.drawInRect_fromRect_operation_fraction_(
        Foundation.NSRect(Foundation.NSPoint(0, 0), Foundation.NSSize(width, height)),
    ...
    return PIL.Image.open(io.BytesIO(bytes(png_data))).convert("RGBA")


def generate_menubar_icons() -> None:
    """Generate 44x44 monochrome template icons for the macOS menu bar."""
    size = 44
    # SVG viewBox is 304x182 (landscape). Fit by width so nothing is cropped.
    render_w = size
    render_h = int(size * 182 / 304)

    for sad in (False, True):
        svg = _build_svg(sad=sad, colour=False, text_stroke_width=4)
        rendered = _svg_to_pil(svg, render_w, render_h)

        img = PIL.Image.new("RGBA", (size, size), (0, 0, 0, 0))
        offset_y = (size - render_h) // 2
        img.paste(rendered, (0, offset_y), rendered)

        suffix = "-sad" if sad else ""
        path = RESOURCES_DIR / f"menubar-icon{suffix}.png"
        img.save(path)
